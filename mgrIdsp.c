...
static void dg_echo(int sockfd, SA *pcliaddr)//, socklen_t clilen)
{
		...
		for ( ; ; ) 
		{
				...
				{	
						int dspip = ntohl( ((struct sockaddr_in *)pcliaddr)->sin_addr.s_addr );
						dspmgr_reset_dsptimer(0xffff , dspip ,3 );
...
void * dspmgr_send_dspkeepalive(void * x)
{
		...
		while(1)
		{
				//p = head;
				//while(p != NULL)
				for(i = 0 ; alldsp[i] != 0 ; i++)
				{
						dspaddr.sin_addr.s_addr = htonl(alldsp[i]);
						Sendto(sockfd , &kp , 6 , 0 ,(struct sockaddr*)&dspaddr , sizeof(dspaddr));
						//printf("-------------------------------------send kp to 0x%x\n",alldsp[i]);
						//p = p->next;
				}
				sleep(1);
		}
